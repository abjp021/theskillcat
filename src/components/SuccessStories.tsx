      ...
      <div className="container mx-auto px-6 lg:px-12">
        ...
        >
          {testimonials.map((testimonial, index) => (
            <div
              key={testimonial.id}
              className={`bg-background border border-border p-6 lg:p-8 transition-all duration-700 hover:shadow-lg ${
                gridVisible
                  ? "opacity-100 translate-y-0"
            ...
            >
              {/* Avatar */}
              <div className="mb-6">
                <div className="w-16 h-16 rounded-full bg-foreground text-background flex items-center justify-center font-display font-bold text-xl">
                  {testimonial.image}
                </div>
              </div>
